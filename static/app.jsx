...
window.initApp = (function () {
    ...
    function App() {
        ...
        useEffect(() => {
            const padTop = 8, padBottom = 8; // keep in sync with inline style below
        ...
        return (
            ...
            <div className="app-container" onClickCapture={flushNow} style={{ touchAction: 'manipulation' }}>
                {/* Dual-layer background as <img> with contain to avoid cropping */}
                <img
                    ...
                    style={{
                        ...
                        objectFit: imgObjectFit, objectPosition: currentObjPos,
                        zIndex: -2,
                ...
                {nextBackground && (
                    <img
                        ...
                        style={{
                            ...
                            width: '100vw', height: '100dvh',
                            maxWidth: '100vw', maxHeight: '100vh',
                            objectFit: imgObjectFit, objectPosition: nextObjPos,
                            zIndex: -1,
                ...
                <div className="game-content" style={{ height: '100%', display: 'flex', flexDirection: 'column', position: 'relative', zIndex: 1 }}>
                    {showRotateHint ? (
                        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.6)', zIndex: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, boxSizing: 'border-box' }}>
                            <div style={{ background: '#111', border: '1px solid #333', borderRadius: 12, padding: 20, maxWidth: 520, width: '90%', color: '#fff', textAlign: 'center' }}>
                                <div style={{ fontSize: 18, marginBottom: 12 }}>For best experience, rotate your device to landscape.</div>
                                <div style={{ opacity: 0.85, marginBottom: 16 }}>Landscape mode shows the full scene and keeps all text and buttons visible.</div>
                                <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
                                    <button className="option-btn" onClick={() => { rotateDismissedRef.current = true; setShowRotateHint(false); }}>Continue in portrait</button>
                                </div>
                            </div>
                        </div>
                    ) : null}
                    ...
                    <div className="content-area" style={{ flex: '1 1 auto', display: 'flex', flexDirection: 'column' }}>
                        ...
                        >
                            {/* Text area - renders fully, no internal scrollbars */}
                            <div id="output" ref={outputRef} style={{ whiteSpace: 'pre-wrap' }}>
                                {log.map((ln, i) => (<div key={i}>{ln}</div>))}
                            </div>
                            {/* Bottom controls: prompt + buttons */}
                            <div ref={bottomRef} style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 8 }}>
                                {prompt ? (
                                    <div>
                                        <input
                                            ref={inputRef}
